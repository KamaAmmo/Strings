...
void def_precision(flags *configs) {
  ...
  if (!configs->point && s21_strchr("eEfgG", configs->specifier)) {
    configs->precision = 6;
  }
}
...
char *parse_spec(char *str, s21_size_t *str_len, va_list *args,
                 flags *configs) {
...
void format_s(char *str, va_list *args, flags *configs) {
  ...
  if (configs->precision > -1)
    s21_strncat(str, string, configs->precision);
  else
...
void int_to_str(flags *configs, char *str, long double number) {
  ...
  if (configs->specifier != 'n') {
    ...
    if (s21_strchr("dioxX", configs->specifier) && configs->precision == 0 &&
...
void float_to_str(char *str, long double number, flags *configs) {
  ...
  if (configs->precision <= 0) {
    ...
    if (s21_strchr("gG", configs->specifier) && configs->precision == 1 &&
        right >= 0.5) {
      left++;
    }
    ...
    if (s21_strchr("gG", configs->specifier)) {
      if (left >= 1) {
        local_precision = configs->precision - (int)s21_strlen(str) + 1 +
                          ((int)roundl(number) == 0);
    ...
    if (s21_strchr("gG", configs->specifier) && (int)roundl(number) != 0) {
      result[configs->precision - (int)s21_strlen(str) + 1] = '\0';
    }
...
void eE_to_str(char *str, long double number, flags *configs) {
  ...
  if (number < 1) {
    ...
    if (s21_strchr("gG", configs->specifier)) delete_zero(configs, str);
    make_mantissa(configs, &str[s21_strlen(str)], notation);
  } else {
    while (number >= 10) {
      notation++;
      number /= 10;
  ...
  if (configs->precision > 0) {
    move_str(&str[1]);
    str[1] = '.';
    for (int i = (int)s21_strlen(str); i < 2 + configs->precision; i++) {
      str[i] = '0';
    }
  }
  if (s21_strchr("gG", configs->specifier)) delete_zero(configs, str);
  make_mantissa(configs, &str[s21_strlen(str)], notation);  // make mantissa
  configs->show_sign = local_sign;
}
...
void make_mantissa(flags *configs, char *mantice, int notation) {
  ...
  if (s21_strchr("eg", configs->specifier)) {
    mantice[start_pos] = 'e';
  } else {
    mantice[start_pos] = 'E';
  }
...
void make_str_flags(flags *configs, char *str) {
  int pos = 0;
  if (!s21_strchr("cuo%%", configs->specifier)) {  //???? \% %% %????
...
void sharp_flag(flags *configs, char *str) {
  int pos = 0;
  if (s21_strchr("oxX", configs->specifier)) {
    if (configs->specifier == 'x' || configs->specifier == 'X') {
      move_str(str);
    }
  ...
  if (s21_strchr("eEf", configs->specifier)) {
    if (!s21_strchr(str, '.')) {
      str[s21_strlen(str)] = '.';
    }
  }
...
void make_str_width(flags *configs, char *str) {
  ...
  if (configs->width > pos_str) {
    ...
    for (pos_str = 0; pos_str <= configs->width; pos_str++) {
      if (s21_strchr("diopuxX", configs->specifier)) {
      ...
      if (s21_strchr("feEgGcs", configs->specifier)) {
...
void make_str_prec(flags *configs, char *str) {
  char *str_prec = s21_NULL;
  str_prec = calloc(1024, sizeof(char));
  int pos = 0;
  if (str_prec != s21_NULL) {
    if (s21_strchr("diouxX", configs->specifier)) {
      while (pos < configs->precision - (int)s21_strlen(str)) {
        str_prec[pos++] = '0';
      }
      for (int i = 0; str[i] != '\0'; i++) {
        str_prec[pos++] = str[i];
      }
      s21_strcpy(str, str_prec);
    }
    free(str_prec);
  }
}
